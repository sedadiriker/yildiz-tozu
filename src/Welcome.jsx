...
export default function Welcome({ audioRef }) {
  ...
  const handleStart = () => {
    ...
    setTimeout(() => setShowTeacherMessage(true), 4000);
        ...
        {showTeacherMessage && (
          ...
          >
            <motion.img
              src={Tuba}
              alt="Tuba Öğretmen"
              className="w-32 h-32 sm:w-60 sm:h-60 rounded-full shadow-lg object-cover z-10 relative mb-6"
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
            ...
            {/* Mesajlar */}
            {[
              "Minik Yıldızlardan Tuba Öğretmenimize...",
              "Işığınızla yolumuzu aydınlattığınız için teşekkürler ✨",
            ].map((line, index) => (
              <motion.span
                key={index}
                ...
                className="block mt-2 text-gradient-to-r from-pink-300 via-purple-300 to-blue-300"
              >
                {line}
              </motion.span>
            ))}
